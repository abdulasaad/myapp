...
import '../../services/profile_service.dart'; // <-- FIX: Import ProfileService for the role check
...
class _CampaignDetailScreenState extends State<CampaignDetailScreen> {
        ...
        content: Form(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                TextFormField(controller: titleController, decoration: const InputDecoration(labelText: 'Task Title'), validator: (value) => (value == null || value.isEmpty) ? 'Required' : null),
                formSpacer,
          ...
          ElevatedButton(
            onPressed: () async {
              if (formKey.currentState!.validate()) {
                ...
                await _addTask(title: titleController.text, description: descriptionController.text, points: int.parse(pointsController.text));
              }
            },
  ...
  Future<void> _addTask({required String title, String? description, required int points}) async {
    try {
      await supabase.from('tasks').insert({'campaign_id': widget.campaign.id, 'title': title, 'description': description, 'points': points});
      if (mounted) {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildSectionHeader('Geofence', onPressed: () {
                Navigator.of(context).push(
                  MaterialPageRoute(builder: (context) => GeofenceEditorScreen(campaign: widget.campaign)),
                );
              },
              buttonLabel: 'Manage',
            ),
            ...
            _buildTasksList(),
            const SizedBox(height: 24),
            _buildSectionHeader('Assigned Agents', onPressed: _showAssignAgentDialog, buttonLabel: 'Assign Agent'),
  ...
  Widget _buildSectionHeader(String title, {required VoidCallback onPressed, required String buttonLabel}) {
    return Row(
      ...
      children: [
        ...
        // --- FIX: This if-condition protects the action button ---
        // It will now only be built and displayed if the user is a manager or admin.
  ...
  Widget _buildTasksList() {
    return FutureBuilder<List<Task>>(
      ...
      builder: (context, snapshot) {
        ...
        if (tasks.isEmpty) return const Center(child: Padding(padding: EdgeInsets.all(16), child: Text('No tasks for this campaign yet.')));
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return Card(
              margin: const EdgeInsets.symmetric(vertical: 4),
              child: ListTile(
                leading: CircleAvatar(child: Text(agent.fullName.substring(0, 1).toUpperCase())),
