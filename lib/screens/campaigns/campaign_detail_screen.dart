...
import '../../models/task.dart'; // <-- NEW IMPORT
...
class _CampaignDetailScreenState extends State<CampaignDetailScreen> {
  ...
  late Future<List<Task>> _tasksFuture; // <-- NEW FUTURE FOR TASKS

  @override
  void initState() {
    super.initState();
    _assignedAgentsFuture = _fetchAssignedAgents();
    _tasksFuture = _fetchTasks(); // <-- INITIALIZE TASK FETCH
  }
  ...
  Widget _buildTasksList() {
    return FutureBuilder<List<Task>>(
      ...
      builder: (context, snapshot) {
        ...
        if (tasks.isEmpty) return const Center(child: Padding(padding: EdgeInsets.all(16), child: Text('No tasks created for this campaign yet.')));
  ...
  Widget _buildAssignedAgentsList() {
    return FutureBuilder<List<AppUser>>(
      ...
      builder: (context, snapshot) {
        ...
        if (agents.isEmpty) return const Center(child: Padding(padding: EdgeInsets.all(16), child: Text('No agents assigned to this campaign yet.')));
