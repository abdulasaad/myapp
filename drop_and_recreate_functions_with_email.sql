-- Drop and recreate functions with email from auth.users table

-- 1. Drop existing function for managers
DROP FUNCTION IF EXISTS get_agents_in_manager_groups(UUID);

-- 2. Create new function for managers with email from auth.users
...
-- 4. Create new function for admins with email from auth.users
