-- Step 2: Create routes table for route visit system (FIXED)
...
-- Temporary policy: Agents can't see routes until we create route_assignments table
-- We'll update this policy in a later step
CREATE POLICY "routes_agent_read_temp" ON public.routes FOR SELECT TO authenticated
USING (false); -- This prevents agents from seeing any routes for now
