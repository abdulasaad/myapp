# Earnings System Database Migration (Complete)

Copy and paste the following SQL code into your Supabase SQL Editor to update the earnings management system:

```sql
-- Add missing columns to existing payments table
ALTER TABLE payments 
ADD COLUMN IF NOT EXISTS touring_task_id UUID REFERENCES touring_tasks(id) ON DELETE SET NULL,
ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW();

-- Update payment_type constraint if needed
ALTER TABLE payments DROP CONSTRAINT IF EXISTS payments_payment_type_check;
ALTER TABLE payments ADD CONSTRAINT payments_payment_type_check 
CHECK (payment_type IN ('task_completion', 'campaign_daily', 'touring_task', 'bonus', 'manual'));

-- Create campaign_daily_participation table (doesn't exist yet)
...
DROP POLICY IF EXISTS "campaign_agents_select_policy" ON campaign_agents;
...
DROP POLICY IF EXISTS "campaign_agents_insert_policy" ON campaign_agents;
...
DROP POLICY IF EXISTS "campaign_agents_update_policy" ON campaign_agents;
...
DROP POLICY IF EXISTS "campaign_agents_delete_policy" ON campaign_agents;
...
DROP TRIGGER IF EXISTS update_campaign_daily_participation_updated_at ON campaign_daily_participation;
CREATE TRIGGER update_campaign_daily_participation_updated_at BEFORE UPDATE ON campaign_daily_participation
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Drop existing functions first
DROP FUNCTION IF EXISTS get_agent_earnings_for_campaign(uuid, uuid);
DROP FUNCTION IF EXISTS get_agent_overall_earnings(uuid);
DROP FUNCTION IF EXISTS update_daily_participation(uuid, uuid, date, decimal, integer, integer);
DROP FUNCTION IF EXISTS auto_update_daily_participation();

-- Function to get agent earnings for a specific campaign
CREATE FUNCTION get_agent_earnings_for_campaign(
    p_agent_id UUID,
    p_campaign_id UUID
) RETURNS JSON AS $$
DECLARE
...
CREATE FUNCTION get_agent_overall_earnings(
    p_agent_id UUID
) RETURNS JSON AS $$
DECLARE
    total_earned INTEGER := 0;
...
CREATE FUNCTION update_daily_participation(
    p_campaign_id UUID,
    p_agent_id UUID,
...
CREATE FUNCTION auto_update_daily_participation()
RETURNS TRIGGER AS $$
DECLARE
