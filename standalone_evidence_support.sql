...
USING (
  ...
  EXISTS (
    ...
    JOIN profiles up ON up.id = evidence.uploader_id
    WHERE ug.user_id = auth.uid()
    AND ug.group_id = ANY(up.group_ids)
    AND EXISTS (
...
-- Create a view for evidence with additional context
CREATE OR REPLACE VIEW evidence_with_context AS
SELECT 
  e.*,
  up.full_name as uploader_name,
  CASE 
    WHEN e.task_assignment_id IS NULL THEN 'standalone'
    ELSE 'task_based'
  END as evidence_type,
  CASE 
    WHEN e.task_assignment_id IS NOT NULL THEN (
      SELECT t.title 
      FROM task_assignments ta 
      JOIN tasks t ON t.id = ta.task_id 
      WHERE ta.id = e.task_assignment_id
    )
    ELSE NULL
  END as task_title,
  CASE 
...
FROM evidence e
JOIN profiles up ON up.id = e.uploader_id;

-- Grant access to the view
GRANT SELECT ON evidence_with_context TO authenticated;
...
COMMENT ON VIEW evidence_with_context IS 'Provides evidence with uploader info and task/campaign context';
